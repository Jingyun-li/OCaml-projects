...
let get_1st lst = match lst with 
  | [] -> 0
  | h::_ -> h
...
let rec regexp_to_nfa (regexp: regexp_t) : (int, char) nfa_t =
  ...
  | Union (a, b) ->
      let s0 = fresh () in
      let s1 = fresh () in
      let left = regexp_to_nfa a in
      let right = regexp_to_nfa b in 
      {sigma = union left.sigma right.sigma;
       qs = s0::s1::(union left.qs right.qs);
       q0 = s0;
       fs = [s1];
       delta = (s0, None, left.q0)::(s0, None, right.q0)::
               ((get_1st left.fs), None, s1)::
               ((get_1st right.fs), None, s1)::(union left.delta right.delta)
      }
  | Concat (a, b) -> 
      let left = regexp_to_nfa a in
      let right = regexp_to_nfa b in 
      {sigma = union left.sigma right.sigma;
       qs = union left.qs right.qs;
       q0 = left.q0;
       fs = [get_1st right.fs]; 
       delta = ((get_1st left.fs), None, right.q0)::(union left.delta right.delta)
      }
  | Star a ->
      let s0 = fresh () in
      let s1 = fresh () in
      let child = regexp_to_nfa a in 
      {sigma = child.sigma;
       qs = s0::s1::child.qs;
       q0 = s0;
       fs = [s1]; 
       delta = ((get_1st child.fs), None, s1)::(s0, None, child.q0)::
               (s1, None, s0)::(s0, None, s1)::child.delta
      }
